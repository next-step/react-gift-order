...
function SlidingOrderCardWrapper({setSelectedCardImg, setSelectedCardTxt, selectedId, setSelectedId}: OrderCardProps) {
    ...
    return (
        ...
        {
            orderCard.map(item => (<SlidingOrderCard src={item.thumbUrl} key={item.id} alt={item.defaultTextMessage} onClick={() => onCardClick(item.id, item.imageUrl, item.defaultTextMessage)} isActive={selectedId === item.id}></SlidingOrderCard>))
        }
        </SlidingOrderCardWrapperStyle>
...
const SlidingOrderCard = styled.img<{isActive: boolean }>`
    ...
    ${({ isActive}) =>
        isActive && `border: 3px solid black;`
    }
`
...
function SenderInputWrapper({setSenderName, senderName} : SenderInputProps) {
    ...
    return (
        <SenderInputWrapperStyle>
            ...
            <SenderInput placeholder="이름을 입력하세요." onChange={(e) => setSenderName(e.target.value)} value={senderName}></SenderInput>
        </SenderInputWrapperStyle>
    );
}
...
// TODO
function ReceiverInputWrapper({receiverName, setReceiverName, receiverPhoneNum, setReceiverPhoneNum, itemCount, setItemCount}: ReceiverInputProps) {
    ...
    return (
        <ReceiverInputWrapperStyle>
            ...
            <ReceiverInputName placeholder="이름을 입력하세요." value={receiverName} onChange={(e) => setReceiverName(e.target.value)}></ReceiverInputName>
            <ReceiverInputPhoneNumberLabel htmlFor="ReceiverInputName">전화번호</ReceiverInputPhoneNumberLabel>
            <ReceiverInputPhoneNumber placeholder="전화번호를 입력하세요" value={receiverPhoneNum} onChange={(e) => setReceiverPhoneNum(e.target.value)}></ReceiverInputPhoneNumber>
            <ReceiverItemNumInputLabel htmlFor="ReceiverInputName">수량</ReceiverItemNumInputLabel>
            <ReceiverItemNumInput placeholder="수량" value={itemCount} onChange={(e)=> setItemCount(parseInt((e.target.value)))}></ReceiverItemNumInput>
        </ReceiverInputWrapperStyle>
    );
}
...
function OrderButton({receiverName, receiverPhoneNum, itemCount}:ButtonProps) {    
...
function Order() {
    ...
    const [selectedCardTxt, setSelectedCardTxt] = useState(orderCard[0].defaultTextMessage);
    ...
    return (
        <ThemeProvider theme={theme}>
            ...
            <Layout>
                <NavBar></NavBar>
                <SlidingOrderCardWrapper setSelectedCardImg={setSelectedCardImg} setSelectedCardTxt={setSelectedCardTxt} selectedId={selectedId} setSelectedId={setSelectedId}></SlidingOrderCardWrapper>
                <CardViewWrapper selectedCardImg={selectedCardImg} selectedCardTxt={selectedCardTxt}></CardViewWrapper>
                <SenderInputWrapper setSenderName={setSenderName} senderName={senderName}></SenderInputWrapper>
                <ReceiverInputWrapper setReceiverName={setReceiverName} receiverName={receiverName} setReceiverPhoneNum={setReceiverPhoneNum} receiverPhoneNum={receiverPhoneNum} setItemCount={setItemCount} itemCount={itemCount}></ReceiverInputWrapper>
                <ItemInfoWrapper selectedItem={selectedItem}></ItemInfoWrapper>
                <OrderButton receiverName={receiverName} receiverPhoneNum={receiverPhoneNum} itemCount={itemCount}></OrderButton>
            </Layout>
        </ThemeProvider>
