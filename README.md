...
[ ] 주문 폼 - 폼 처리
      ...
      [ ] 3단계 - 단체 주문 기능 구현
            - 재사용 가능성이 높은 컴포넌트, Hooks가 보이면 재사용성을 고려하여 만듦.
                  -> ReceiverForm, ReceiverTable, EmptyReceiverBox 컴포넌트를 분리해 재사용성 향상
                  -> 입력 상태 및 유효성 관리를 위해 useReceivers 커스텀 훅으로 분리
            - 주문하기 페이지의 폼을 React Hook Form를 사용해서 리팩터링함. (form 데이터를 useState로 다루는 경우는 없게 해야 함.)
            ...
            [ ] (선택) 유효성 검사를 Zod 라이브러리를 사용해서 처리해야 함.